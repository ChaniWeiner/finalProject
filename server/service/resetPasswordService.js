// import { executeQuery } from '../config/database';
// import crypto from 'crypto';
// import bcrypt from 'bcrypt';
// import { sendPasswordChangeEmail } from './email.js';

// const saltRounds = 10;

// export class RsesetPasswordService {
//     // פונקציה ליצירת קוד OTP
//     static generateOtp(length = 6) {
//         return crypto.randomBytes(length).toString('hex').slice(0, length).toUpperCase();
//     }

//     static async sendOtp(userId) {
//         //const otp = new RsesetPasswordService.generateOtp(); // יצירת קוד OTP

//         // עדכון OTP בטבלה
//         const updateOtpQuery = 'UPDATE passwords SET OTP = ? WHERE userId = ?';
//         await executeQuery(updateOtpQuery, [otp, userId]);

//         // קבלת המייל של המשתמש
//         const emailQuery = 'SELECT email FROM users WHERE id = ?';
//         const user = await executeQuery(emailQuery, [userId]);
//         const email = user[0].email;

//         // שליחת המייל עם ה-OTP
//         await sendPasswordChangeEmail(email, otp);
//     }

//     static async verifyOtp(userId, otp, newPassword) {
//         const selectOtpQuery = 'SELECT OTP FROM passwords WHERE userId = ?';
//         const result = await executeQuery(selectOtpQuery, [userId]);

//         if (result[0].OTP !== otp) {
//             return false; // OTP לא תואם
//         }

//         const hashedPassword = await bcrypt.hash(newPassword, saltRounds);
//         const updatePasswordQuery = 'UPDATE passwords SET password = ?, OTP = NULL WHERE userId = ?';
//         await executeQuery(updatePasswordQuery, [hashedPassword, userId]);

//         return true; // סיסמה הוזנה בהצלחה
//     }
// }




import { executeQuery } from './query.js';
import crypto from 'crypto';
import bcrypt from 'bcrypt';
import { sendPasswordChangeEmail } from './email.js';

const saltRounds = 10;

export class ResetPasswordService {
    // פונקציה ליצירת קוד OTP
   generateOtp(length = 6) {
        return crypto.randomBytes(length).toString('hex').slice(0, length).toUpperCase();
    }

    async sendOtp(userId) {
        const otp = new ResetPasswordService(); // יצירת קוד OTP
        const otp1 = otp.generateOtp();
        // עדכון OTP בטבלה
        const updateOtpQuery = 'UPDATE db_fp.passwords SET OTP = ? WHERE userId = ?';
        await executeQuery(updateOtpQuery, [otp1, userId]);

        // קבלת המייל של המשתמש
        const emailQuery = 'SELECT email FROM db_fp.users WHERE userId = ?';
        const user = await executeQuery(emailQuery, [userId]);
        const email = user[0].email;

        // שליחת המייל עם ה-OTP
        await sendPasswordChangeEmail(email, otp1);
    }

    async verifyOtp(userId, otp, newPassword) {
        const selectOtpQuery = 'SELECT OTP FROM db_fp.passwords WHERE userId = ?';
        const result = await executeQuery(selectOtpQuery, [userId]);

        if (result[0].OTP !== otp) {
            return false; // OTP לא תואם
        }

        //const hashedPassword = await bcrypt.hash(newPassword, saltRounds);
        const updatePasswordQuery = 'UPDATE db_fp.passwords SET password = ?, OTP = NULL WHERE userId = ?';
        //await executeQuery(updatePasswordQuery, [hashedPassword, userId]);
        await executeQuery(updatePasswordQuery, [newPassword, userId]);
        return true; // סיסמה הוזנה בהצלחה
    }
}

export default ResetPasswordService;
